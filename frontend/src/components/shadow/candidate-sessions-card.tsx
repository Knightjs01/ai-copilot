...
import {
  ...
  useRevokeCandidateSession,
  useRevokeOtherCandidateSessions,
} from "@/lib/queries/candidate-security";

// Mirrors components/security/sessions-card.tsx (the company equivalent) exactly, against
// /candidate-auth/sessions* instead of /auth/sessions*.
...
export function CandidateSessionsCard() {
  const { data: sessions, isLoading } = useCandidateSessions();
  const revokeSession = useRevokeCandidateSession();
  const revokeOthers = useRevokeOtherCandidateSessions();
