...
export function PassportWizard() {
  ...
  React.useEffect(() => {
    ...
    if (passport?.current_version_number != null) {
      ...
      if (!firstName) resumeStep = 1;
      else if (!headline || !summary) resumeStep = 2;
      else if (!location || !remotePreference) resumeStep = 3;
      else resumeStep = 4;
    }
  ...
  const handleRequestSkillsSuggestion = async () => {
    setSkillsSuggestionError(null);
    try {
      const result = await suggestSkills.mutateAsync({
        headline: headline || null,
        summary: summary || null,
        existing_skills: skillsText.split(",").map((s) => s.trim()).filter(Boolean),
      });
  ...
  if (firstName) completedIndices.add(1);
  if (headline && summary) completedIndices.add(2);
  if (location && remotePreference) completedIndices.add(3);
      ...
      <div>
        {activeStep === 0 && (
          <UploadCvStep
            ...
            parsedSummary={
              hasParsedData
                ? { headline: headline || null, skillsCount: skillsArray.length, entriesCount: careerEntries.length }
                : null
            }
        ...
        {activeStep === 2 && (
          <ProfessionalProfileStep
            ...
            onRequestSkillsSuggestion={handleRequestSkillsSuggestion}
            ...
            onDismissSkillsSuggestions={() => setSkillsSuggestions(null)}
          />
        )}
      ...
      <div className="flex items-center justify-between gap-3 border-t border-border pt-5">
        ...
        <div className="flex items-center gap-3">
          ...
          {activeStep < STEPS.length - 1 ? (
            <Button type="button" variant="brand" onClick={handleContinue} disabled={savePassport.isPending}>
